...
public class SensorDataResourceIntTest {
    ...
    public void createSensorData() throws Exception {
        ...
        restSensorDataMockMvc.perform(post("/api/sensorDatas")
        ...
        assertThat(sensorDatas).hasSize(databaseSizeBeforeCreate + 1);
        SensorData testSensorData = sensorDatas.get(sensorDatas.size() - 1);
    ...
    public void checkTimeIsRequired() throws Exception {
        ...
        List<SensorData> sensorDatas = sensorDataRepository.findAll();
        assertThat(sensorDatas).hasSize(databaseSizeBeforeTest);
    }
    ...
    @Test
    @Transactional
    public void getAllSensorDatas() throws Exception {
        // Initialize the database
        sensorDataRepository.saveAndFlush(sensorData);

        // Get all the sensorDatas
        restSensorDataMockMvc.perform(get("/api/sensorDatas?sort=id,desc"))
    ...
    public void getSensorData() throws Exception {
        ...
        // Get the sensorData
        restSensorDataMockMvc.perform(get("/api/sensorDatas/{id}", sensorData.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    public void getNonExistingSensorData() throws Exception {
        // Get the sensorData
        restSensorDataMockMvc.perform(get("/api/sensorDatas/{id}", Long.MAX_VALUE))
    ...
    public void updateSensorData() throws Exception {
        ...
        restSensorDataMockMvc.perform(put("/api/sensorDatas")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(sensorData)))
                .andExpect(status().isOk());
        ...
        // Validate the SensorData in the database
        List<SensorData> sensorDatas = sensorDataRepository.findAll();
        assertThat(sensorDatas).hasSize(databaseSizeBeforeUpdate);
        SensorData testSensorData = sensorDatas.get(sensorDatas.size() - 1);
    ...
    public void deleteSensorData() throws Exception {
        ...
        // Get the sensorData
        restSensorDataMockMvc.perform(delete("/api/sensorDatas/{id}", sensorData.getId())
                .accept(TestUtil.APPLICATION_JSON_UTF8))
                .andExpect(status().isOk());

        // Validate the database is empty
        List<SensorData> sensorDatas = sensorDataRepository.findAll();
        assertThat(sensorDatas).hasSize(databaseSizeBeforeDelete - 1);
    }
}
